...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    /**
     * Check if user is same provider appointment
     */
    if (provider_id === req.userId) {
      return res.status(400).json({
        error: { message: 'You cannot create appointment for yourself' },
      });
    }
    ...
    if (isBefore(hourStart, new Date())) {
      return res
        .status(400)
        .json({ error: { message: 'Past date are not permitted' } });
    }
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error: {
          message: 'You can only cancel appointment 2 hours is advance',
        },
      });
    }
